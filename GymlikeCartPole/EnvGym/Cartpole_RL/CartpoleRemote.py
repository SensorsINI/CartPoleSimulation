...
--------
    • The hardware (client) opens a DEALER/REQ socket and **sends**
      one JSON frame:
          ...
          {"state": [...], "time": t, "updated_attributes": {...}}
...
from CartPole.data_generator import random_experiment_setter   # only for params
...
class Cartpole_Remote(CartPoleSimulatorBase):
    ...
    # ------------------------- helper I/O ----------------------------
    def _recv_state(self) -> Tuple[bytes, np.ndarray]:
        """
        Blocks until a state frame arrives.
        Returns (identity, state ∈ ℝ⁶).
        """
    ...
    def _send_action(self, identity: bytes, action: np.ndarray) -> None:
        """
        Reply with the control command encoded as ``{"Q": [u]}``.
        The hardware understands exactly this format.
        """
        u      = float(np.squeeze(action))          # ensure JSON-serialisable
        frame  = json.dumps({"Q": [u]}).encode("utf-8")
    ...
    def reset(self) -> None:
        ...
        **No reply is sent yet** – the first action will be dispatched
        ...
        """
        self._pending_identity, self._pending_state = self._recv_state()
    ...
    def next_state(self, _current_state: np.ndarray,
        ...
        self._send_action(self._pending_identity, action)
