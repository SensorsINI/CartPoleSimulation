...
class controller_mppi_var(template_controller):
    def __init__(self, seed: int, num_control_inputs: int, cc_weight: float, R: float, LBD_mc: float, mpc_horizon: float, num_rollouts: int, dt: float, predictor_intermediate_steps: int, NU_mc: float, SQRTRHOINV_mc: float, GAMMA: float, SAMPLING_TYPE: str, NET_NAME: str, predictor_name: str, cost_function: str, LR: float, max_grad_norm: float, STDEV_min: float, STDEV_max: float, interpolation_step: int, **kwargs):
    ...
    def uncorr_cost(self, s_hor ,u, target_position, u_prev, delta_u):
        stage_cost = self.q(s_hor[:,1:,:],u,target_position, u_prev)
        stage_cost = stage_cost
        unc_cost = tf.math.reduce_sum(stage_cost,axis=1)
        unc_cost = unc_cost + self.phi(s_hor, target_position)
        return unc_cost
    ...
    def do_step(self, s, target_position, u_nom, random_gen, u_old, nuvec):
        ...
        with tf.GradientTape(watch_accessed_variables=False) as tape:
            ...
            unc_cost = self.uncorr_cost(rollout_trajectory, u_run, target_position, u_old, delta_u)
    ...
    def step(self, s: np.ndarray, target_position: np.ndarray, time=None):
        ...
        self.u, self.u_nom, new_nuvec, u_run, traj_cost = self.do_step(s, target_position, self.u_nom, self.rng_mppi, self.u, self.nuvec)
