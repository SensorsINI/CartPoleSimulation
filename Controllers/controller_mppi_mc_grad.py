...
def mppi_correction_cost(u, delta_u, nuvec):
    return cc_weight * (0.5 * (1 - 1.0 / nuvec**2) * R * (delta_u ** 2) + R * u * delta_u + 0.5 * R * (u ** 2))
...
def inizialize_pertubation(random_gen, nuvec ):
    delta_u = random_gen.normal([num_rollouts, mppi_samples], dtype=tf.float32) * nuvec*SQRTRHODTINV
    return delta_u



#controller class
class controller_mppi_mc_grad(template_controller):
    def __init__(self):
        ...
        self.nuvec = np.math.sqrt(NU)*tf.ones([1, mppi_samples])
    ...
    def controller_reset(self):
        self.u_nom = tf.zeros([1, mppi_samples], dtype=tf.float32)
        self.nuvec.assign(SQRTRHODTINV*np.math.sqrt(NU)*tf.ones([1, mppi_samples]))
        self.u = 0.0
