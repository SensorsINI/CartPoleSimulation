...
opt = tf.keras.optimizers.RMSprop(learning_rate=cem_LR/10)
...
def distance_difference_cost(position, target_position):
    ...
    # , tf.float32) * 1.0e9  # Soft constraint: Do not crash into border
    return ((position - target_position) / (2.0 * TrackHalfLength)) ** 2 + 125*(tf.exp(125*(position-0.9*TrackHalfLength)) + tf.exp(125*(-position-0.9*TrackHalfLength)))
...
#cem class
class controller_dist_opt(template_controller):
    def __init__(self):
    ...
    def step(self, s: np.ndarray, target_position: np.ndarray, time=None):
        ...
        RMS = weights[1]
        RMSN = tf.concat([RMS[:, 1:], tf.zeros([num_rollouts,1])], -1)
        new_weights = [weights[0], RMSN]
