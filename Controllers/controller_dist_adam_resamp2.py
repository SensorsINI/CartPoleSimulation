...
class controller_dist_adam_resamp2(template_controller):
    def __init__(self, seed: int, num_control_inputs: int, cost_function: str, dt: float, mpc_horizon: float, num_rollouts: int, outer_its: int, sample_stdev: float, resamp_per: int, predictor_name: str, predictor_intermediate_steps: int, NET_NAME: str, SAMPLING_TYPE: str, interpolation_step: int, warmup: bool, cem_LR: float, opt_keep_k: int, gradmax_clip: float, adam_beta_1: float, adam_beta_2: float, adam_epsilon: float, **kwargs):
        ...
        self.cost = getattr(importlib.import_module(f"others.cost_functions.{cost_function}"), "cost")
        ...
        #warmup setup
        self.first_iter_count = outer_its
        if self.do_warmup:
    ...
    def step(self, s: np.ndarray, target_position: np.ndarray, time=None):
        ...
        for _ in range(0, iters):
            Qn = self.grad_step(s, target_position, self.Q_tf, self.opt)
            self.Q_tf.assign(Qn)

        #retrieve optimal input and prepare warmstart
        self.u, self.dist_mue, self.stdev, Qn, self.bestQ, J = self.get_action(s, target_position, self.Q_tf)
