...
class cartpole_dancer_cost(cost_function_base):
    ...
    def get_stage_cost(self, states: TensorType, inputs: TensorType, previous_input: TensorType, time:float=None):
        ...
        if not self.lib.equal(self.policy_number,2): # if not spin from 'spin2' as policy
            ...
            spin_cost_energy = -self.pole_energy_total_weight * (self.pole_energy_kinetic(angleD_states) + self.pole_energy_potential(cosangle_states))
            spin_cost_spin = gui_target_equilibrium* self.pole_swing_weight * self.dist(angleD_states)
            stage_costs = spin_cost_energy + spin_cost_spin
    ...
    def get_terminal_cost(self, terminal_states: TensorType):
        ...
        if  not self.lib.equal(self.policy_number,2): # if not spin cost
            ...
            terminal_cost_energy = -self.pole_energy_total_weight*(self.pole_energy_kinetic(terminal_state_angled)+self.pole_energy_potential(terminal_state_cosangle))
    ...
    def pole_energy_potential(self, cosangle):
        ...
        :returns: the energy
        """
        return cosangle*m_pole*L*9.8*0.5  # hopefully this is height of COM potential gravitational energy
