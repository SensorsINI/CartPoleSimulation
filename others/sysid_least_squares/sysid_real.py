...
def filt_data(file):
    b, a = signal.butter(2, 0.064)

    for name in ['position', 'positionD']:
        y[:, n_to_c[name]] = signal.medfilt(y[:, n_to_c[name]], kernel_size=5)
        y[:, n_to_c[name]] = signal.filtfilt(b, a, y[:, n_to_c[name]], padlen=150)

    y[:, n_to_c['angle']] = signal.medfilt(y[:, n_to_c['angle']], kernel_size=5)


    sin_a = np.sin(y[:, n_to_c['angle']])
    cos_a = np.cos(y[:, n_to_c['angle']])

    sin_a_filt = signal.filtfilt(b, a, sin_a, padlen=150)
    cos_a_filt = signal.filtfilt(b, a, cos_a, padlen=150)
    ...
    y[:, n_to_c['angleD']] = signal.filtfilt(b, a, da_array, padlen=150)
...
file_start_index = 5000
file_end_index = 6000
file_animate = True

#file = 'cartpole-2021-07-13-17-59-28.csv'
#file_start_index = 1000
#file_end_index = 1500
#file_animate = False
...
bounds = ([0.001, 0.001, 0.001, 0.001, 0.001, 0.001], [1E5, 1E5, 1E5, 1E5, 1E5, 1E5])

res = least_squares(lambda p: cartpole_residuals_multi_step(y, p, file_start_index, file_end_index, steps=15), p_0, bounds=bounds, verbose=2, loss='soft_l1', f_scale=0.1)

for i in range(20, 100, 10):
    print('steps: {}'.format(i))
    res = least_squares(lambda p: cartpole_residuals_multi_step(y, p, file_start_index, file_end_index, steps=i), res.x, bounds=bounds, verbose=2, loss='soft_l1', f_scale=0.1)
