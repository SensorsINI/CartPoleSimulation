...
def q_rev(s,u,target_position, u_prev):
...
def cost_rev(s_hor ,u,target_position,u_prev):
    stage_cost = q_rev(s_hor[:,1:,:],u,target_position,u_prev)
    total_cost = tf.math.reduce_sum(stage_cost,axis=1)
    total_cost = total_cost + phi_rev(s_hor,target_position)
    return total_cost
