...
from SI_Toolkit_ApplicationSpecificFiles.predictor_euler_Diego import predictor_euler_Diego
...
def grad_desc(predictor, u, s):
    with tf.GradientTape() as tape:
        tape.watch(u)
        rollout_trajectory = predictor.predict(s, u)
...
# @tf.function(jit_compile = True)
def test(predictor, u, s):
    for i in range(0, 100):
        dc_du, cost = grad_desc(predictor, u, s)
        print(cost)
        u = u - lr * dc_du
...
predictor = predictor_euler_Diego(horizon=cem_samples, dt=dt, max_steps=10, batch_size=num_rollouts)
predictor_2 = predictor_ODE_tf_pure(horizon=cem_samples, dt=dt, intermediate_steps=10)
...
traj2 = predict_wrap(predictor_2, s, u)
lr = 10
...
f_to_measure = 'test(predictor,u,s)'
