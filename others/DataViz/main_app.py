...
class MainApplication(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.feature_filter_max_var = tk.StringVar(value="")  # FIX: define this var
    ...
    def update_plot(self):
        ...
        if col_to_filter and (cf_min is not None) and (cf_max is not None) and (cf_min < cf_max):
            df_filtered = df_filtered[(df_filtered[col_to_filter] >= cf_min) & (df_filtered[col_to_filter] <= cf_max)]
        ...
        if feat_col in df_filtered.columns:
            ...
            if f_min is not None and f_max is not None and f_min < f_max:
                df_filtered = df_filtered[(df_filtered[feat_col] >= f_min) & (df_filtered[feat_col] <= f_max)]
        ...
        if plot_type == "Scatter":
            ...
            if plot_option == "Density":
                ...
                if c_data is not None:
                    stat, x_edges, y_edges, _ = binned_statistic_2d(x_vals, y_vals, c_data,
    ...
    def _get_cdata_for_plot_option(self, option, df, for_pls=False):
        def get_scol_clipped(scol, dframe):
            if self.clip_control_var.get():
                ...
                return dframe[clipped].values if clipped in dframe.columns else dframe[scol].clip(-1, 1).values
            else:
